"""
HSE CCTV Detector — نسخه نهایی
قوانین فعال:
  ✅ سقوط کارگر
  ✅ آتش/حریق
  ✅ بی‌حرکتی (احتمال بیهوشی)
  ✅ برخورد خودرو-کارگر
"""
...
# ═══════════════════════════════════════════════════════════
print("=" * 70)
print("🚀 HSE Detector — سقوط | آتش | بی‌حرکتی | برخورد")
print("=" * 70)

model = YOLO("yolov8s.pt")
cap   = cv2.VideoCapture(VIDEO_SOURCE)
fps   = cap.get(cv2.CAP_PROP_FPS) or 25
log.info(f"ویدیو: {VIDEO_SOURCE} | FPS: {fps:.0f}")
...
def check_immobility(pid, now):
    ...
    truly    = dist < IMMOBILITY_PIXEL and mid_dist < IMMOBILITY_PIXEL
    return (truly and dur > IMMOBILITY_SEC), dur

def send_incident(key, title, inc_type, severity, frame):
    if not can_send(key):
        remaining = COOLDOWN.get(key, 30) - (time.time() - last_sent[key])
        log.debug(f"⏳ {key} cooldown — {remaining:.0f}s")
        return False
    try:
        payload = {
            "title":       title,
            ...
            "description": f"تشخیص AI ({CONFIRM_FRAMES} فریم تأیید) | {title}"
        }
...
while True:
    ...
    for r in results:
        ...
        for box in r.boxes:
            ...
            if cls == "person":
                ...
                cv2.putText(frame,f"{cls} {conf:.0%}#{tid}",(x1,y1-4),
    ...
    if update_confirm("fall", fall_now):
        ...
        send_incident("fall","حادثه: سقوط و زمین‌خوردن کارگر",
                      "Accident","High",frame)

    # ── ۲. آتش ──────────────────────────────────────────
    fire_ok, fire_px = detect_fire(frame)
    if update_confirm("fire", fire_ok):
        send_incident("fire","حادثه: تشخیص آتش/حریق",
                      "Accident","High",frame)

    # ── ۳. بی‌حرکتی ─────────────────────────────────────
    for p in persons:
        is_still, dur = check_immobility(p['id'], now)
        pid_key = f"immobility_{p['id']}"
        if update_confirm(pid_key, is_still):
    ...
    if update_confirm("collision", col_now):
        send_incident("collision","حادثه: احتمال برخورد لیفتراک با کارگر",
                      "NearMiss","High",frame)

    # ── وضعیت روی فریم ──────────────────────────────────
    ts = datetime.now().strftime("%H:%M:%S")
    cv2.putText(frame,f"HSE AI | {len(persons)}p {len(vehicles)}v | {ts}",
               (10,25),cv2.FONT_HERSHEY_SIMPLEX,0.55,(0,255,200),1)

    for i,(key,cd) in enumerate(COOLDOWN.items()):
        rem = max(0, cd-(now-last_sent[key]))
        cv2.putText(frame,
            f"{key}: {'ready' if rem==0 else f'{rem:.0f}s'}",
            (10,45+i*16),cv2.FONT_HERSHEY_SIMPLEX,0.35,
            (0,255,0) if rem==0 else (100,100,100),1)
